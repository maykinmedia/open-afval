...
2. Get the code:
   ...
   .. code-block:: bash
       ...
       git clone git@bitbucket.org:maykinmedia/openafval.git
       cd openafval
...
``src/openafval/conf/local.py``. You can base this file on the
...
.. code-block:: bash
    ...
    python src/manage.py test openafval
...
* ``DB_NAME``: name of the database for the project. Defaults to ``openafval``.
* ``DB_USER``: username to connect to the database with. Defaults to ``openafval``.
* ``DB_PASSWORD``: password to use to connect to the database. Defaults to ``openafval``.
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       docker-compose up -d
       Starting openafval_db_1 ... done
       Starting openafval_web_1 ... done
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       docker exec -it openafval_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       docker exec -it openafval_web_1 /app/src/manage.py loaddata admin_index groups
...
.. _Github: https://github.com/maykinmedia/open-afval/
...
environment variables. See ``src/openafval/conf/docker.py`` for
all settings.

.. code-block:: bash

    docker build -t openafval
    docker run \
        -p 8000:8000 \
        -e DATABASE_USERNAME=... \
        -e DATABASE_PASSWORD=... \
    ...
    docker exec -it openafval /app/src/manage.py createsuperuser
