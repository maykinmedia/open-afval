...
1. ``openafval-django``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``openafval-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

**develop** branch
------------------

1. ``openafval-django-develop``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``openafval-django-develop-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

pull requests
-------------
1. ``openafval-pr-django``: backend tests, runs ``./bin/jenkins_django_pr.sh``.
2. ``openafval-pr-js``: frontend tests, runs ``./bin/jenkins_js.sh``.
3. ``openafval-pr-sass``: checks that sass compiles, runs ``./bin/jenkins_sass.sh``.
4. ``openafval-pr-isort``: checks that imports are correctly
