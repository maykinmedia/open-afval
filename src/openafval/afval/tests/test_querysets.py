...
class ContainerQuerySetTest(TestCase):
    ...
    def test_with_aggregate_weights_adds_annotation(self):
        klant = KlantFactory.create()
        container = ContainerFactory.create()
        location = ContainerLocationFactory.create()

        LedigingFactory.create(
            klant=klant, container=container, container_location=location, gewicht=10.0
        )
        LedigingFactory.create(
            klant=klant, container=container, container_location=location, gewicht=20.0
        )

        result = Container.objects.for_klant(klant).with_aggregate_weights(klant=klant)

        self.assertEqual(result.count(), 1)
        self.assertEqual(result.first().totaal_gewicht, 30.0)

    def test_with_aggregate_weights_respects_date_filter(self):
        klant = KlantFactory.create()
        container = ContainerFactory.create()
        location = ContainerLocationFactory.create()
        ...
        LedigingFactory.create(
            ...
            container_location=location,
            gewicht=30.0,
            geleegd_op=datetime(2026, 2, 5, tzinfo=ZoneInfo(TZ_LOCAL)),
        )

        result = Container.objects.for_klant(klant).with_aggregate_weights(
            klant=klant,
    ...
    def test_with_aggregate_weights_isolates_between_klanten(self):
        klant1 = KlantFactory.create()
        klant2 = KlantFactory.create()
        container = ContainerFactory.create()
        location = ContainerLocationFactory.create()
        ...
        LedigingFactory.create(
            klant=klant1, container=container, container_location=location, gewicht=10.0
        )
        LedigingFactory.create(
            klant=klant2, container=container, container_location=location, gewicht=50.0
        )

        result = Container.objects.for_klant(klant1).with_aggregate_weights(klant=klant1)

        self.assertEqual(result.first().totaal_gewicht, 10.0)
...
class ContainerLocationQuerySetTest(TestCase):
    ...
    def test_with_aggregate_weights_adds_annotation(self):
        ...
        LedigingFactory.create(
            klant=klant, container=container, container_location=location, gewicht=15.0
        )
        LedigingFactory.create(
            klant=klant, container=container, container_location=location, gewicht=25.0
        )

        result = ContainerLocation.objects.for_klant(klant).with_aggregate_weights(klant=klant)

        self.assertEqual(result.count(), 1)
        self.assertEqual(result.first().totaal_gewicht, 40.0)
    ...
    def test_with_aggregate_weights_respects_date_filter(self):
        ...
        LedigingFactory.create(
            ...
            container_location=location,
            gewicht=20.0,
            geleegd_op=datetime(2026, 2, 10, tzinfo=ZoneInfo(TZ_LOCAL)),
        )

        result = ContainerLocation.objects.for_klant(klant).with_aggregate_weights(
            klant=klant,
            start_date="2026-01-01",
            end_date="2026-01-31",
        )
